...
class PetBreedClassifier(nn.Module):
    def __init__(self):
        ...
        self.head = nn.Sequential(
            nn.Conv2d(4, 80, kernel_size=7, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(80),
        ...
        self.stages = nn.Sequential(
            self._createStage(80, 80, blocks=2, stride=1),
            self._createStage(80, 160, blocks=2, stride=2),
            self._createStage(160, 320, blocks=2, stride=2),
            self._createStage(320, 640, blocks=2, stride=2),
        )
